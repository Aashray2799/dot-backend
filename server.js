...
// ULTRA-SIMPLE TEST - Just return room_inventory data
...
// Get all available rooms with current pricing - FIXED TO INCLUDE MOTEL DATA
app.get('/api/rooms', async (req, res) => {
    try {
        // First get the room data
        const roomQuery = `SELECT * FROM room_inventory WHERE status = 'active'`;
        const roomResult = await pool.query(roomQuery);
        
        // Then get the motel data
        const motelQuery = `SELECT * FROM motels WHERE id = 1`;
        const motelResult = await pool.query(motelQuery);
        
        // Combine the data
        const rooms = roomResult.rows.map(room => ({
            ...room,
            motel_name: motelResult.rows[0]?.namw || 'Signal Hill Motel',
            motel_address: motelResult.rows[0]?.address || 'Signal Hill, CA',
            total_rooms: motelResult.rows[0]?.total_rooms || 15,
            active_bookings: 0
        }));
